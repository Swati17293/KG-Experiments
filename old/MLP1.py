...
def train_save_model():
    ...
    gate_model = Dense(128, activation='relu')(embed_title) #selu #elu=41
    ...
    if os.path.isfile('MLP1/MODEL.h5') == False:
        ...
        with open('MLP1/MODEL.json', "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        gate_model.save_weights('MLP1/MODEL.h5')
        print("\nSaved model to disk...\n")
    else: 
        print('\nLoading model...')  
        # load json and create model
        json_file = open('MLP1/MODEL.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        gate_model = model_from_json(loaded_model_json)
        # load weights into new model
        gate_model.load_weights('MLP1/MODEL.h5', by_name=True) 

    print('\n\nGenerating answers...') 
    ans = gate_model.predict([testque_feature])

    fp = open('MLP1/test.ans', 'w')
