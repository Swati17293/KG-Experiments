...
def train_save_model():
    ...
    tokenizer_q = text.Tokenizer(num_words=500)
    tokenizer_q.fit_on_texts(title_list_all)
    wrdidx_title = tokenizer_q.word_index

    tokenizer_s = text.Tokenizer(num_words=100)
    ...
    embed_summary = Flatten()(embed_summary)
    ...
    if os.path.isfile('MLP3/MODEL.h5') == False:
        ...
        with open('MLP3/MODEL.json', "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        gate_model.save_weights('MLP3/MODEL.h5')
        print("\nSaved model to disk...\n")
    else: 
        print('\nLoading model...')  
        # load json and create model
        json_file = open('MLP3/MODEL.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        gate_model = model_from_json(loaded_model_json)
        # load weights into new model
        gate_model.load_weights('MLP3/MODEL.h5', by_name=True) 

    print('\n\nGenerating answers...') 
    ans = gate_model.predict([testque_feature, testsum_feature])

    fp = open('MLP3/test.ans', 'w')
